...
extern void avx_clear2 (__m128d, __m128d);
extern void sse_clear (__m128d, __m128d);
extern void sse_clear2 (__m128d, __m128d);
...
main ()
{
  ...
  start = __rdtscp (&aux);
  for (i = 0; i < LOOP; i++)
    sse_clear2 (xmm0[i], xmm1[i]);
  end = __rdtscp (&aux);
  diff = end - start;

  printf ("sse_clear2: %lld\n", diff);
  ...
  start = __rdtscp (&aux);
  for (i = 0; i < LOOP; i++)
    avx_clear2 (xmm0[i], xmm1[i]);
  end = __rdtscp (&aux);
  diff = end - start;

  printf ("avx_clear2: %lld\n", diff);
